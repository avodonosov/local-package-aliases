...
* SLIME support
  ...
  therefore it doesn't undersdand our aliases.

  In result SLIME symbol completion, slime-edit-definition, function
  argument list hints do not work out of box for aliased tokens like =$lib:func=.
  ...
  So, during dynamic extend of every slime request evaluatoin,
  the aliases become real package nicknames and SLIME can handle them
  as usually.
  ...
  To have SIME support enabled automatically add the following
  to your /~\/.swank.lisp/:
#+BEGIN_SRC common-lisp
